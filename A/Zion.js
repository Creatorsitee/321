...
module.exports = Zion = async (Zion, m, chatUpdate, store) => {
    try {
        const body = (
        ...
        const sender = m.key.fromMe ? Zion.user.id.split(":")[0] + "@s.whatsapp.net" || Zion.user.id
: m.key.participant || m.key.remoteJid;
        ...
        const botNumber = await Zion.decodeJid(Zion.user.id);
        ...
        const groupMetadata = isGroup ? await Zion.groupMetadata(m.chat).catch((e) => {}) : "";
...
buttons: [
  ...
  viewOnce: true,
image: { url: thumbnail },
caption: `Hallo Kak *${pushname}*, Perkenalkan nama saya *${namaBot}*, Saya
adalah Bot WhatsApp yang diciptakan oleh *${namaowner}*, siap membantu kapanpun
dan dimanapun 🫡
...
newsletterJid: idch,
newsletterName: ` Haii ${pushname}`,
serverId: 200
}, 
externalAdReply: {
title: ` ${namaBot}`, 
thumbnailUrl: thumbnaill, 
renderLargerThumbnail: false, 
mediaType: 1, 
previewType: 1, 
sourceUrl: "https://robot-[🤖]-pedia.ai", 
}}
}, {quoted: qkontak })
}
//
...
//theme sticker reply
        ...
        if (m.message) {
            ...
            console.log(chalk.bgHex("#000000").bold(`▢ New chat sender `));
            console.log(
...
return await Zion.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
}
}
...
if (
  ...
  if (isPossibleBot) {
    ...
    if (warnCount <= 2) { // max warn ubah ae
      await Zion.sendMessage(groupId, {
        text: `⚠️ *Antibot aktif!*\nBot tidak diizinkan di grup ini!\nPeringatan ke-${warnCount}/3`,
        mentions: [sender]
      })
      await Zion.sendMessage(groupId, { delete: m.key }).catch(() => {})
    } else {
      await Zion.sendMessage(groupId, {
        text: `Bot @${sender.split('@')[0]} telah dikeluarkan dari grup karena melanggar aturan.`,
        mentions: [sender]
      })
      await Zion.groupParticipantsUpdate(groupId, [sender], 'remove').catch(() =>
        Zion.sendMessage(groupId, { text: '⚠️ Gagal kick bot. Mungkin bot bukan admin.' })
      )
...
//
   ...
   const fitur1 = () =>{
            var mytext = fs.readFileSync("./Zion.js").toString()
...
if (command) { Zion.readMessages([m.key])}
Zion.sendPresenceUpdate('composing', from)
}
if (global.autoread) {
Zion.readMessages([m.key])
        };
...
Zion.enhancer = Zion.enhancer ? Zion.enhancer : {};
        
Zion.autoshalat = Zion.autoshalat ? Zion.autoshalat : {}
    let id = m.chat
    if (id in Zion.autoshalat) {
    return false
    }
    let jadwalSholat = {
    shubuh: '04:05',
...
contextInfo: {
    externalAdReply: {
        ...
        body: `🕑 ${waktu}`,
        sourceUrl: '',
        thumbnailUrl: "https://img1.pixhost.to/images/6601/612882514_Zion.jpg",
        renderLargerThumbnail: true
    }
}
            }, {}),
            setTimeout(async () => {
delete Zion.autoshalat[m.chat]
            }, 5000)
        ]
    }
    }
...
global.menfess = global.menfess ? global.menfess : {}
         ...
         if (mf && body) {
          ...
          //  if (!/conversation|extended/.test(m.type)) return reply(`Balas dengan teks biasa.`)
            ...
            await Zion.sendMessage(mf.from, { text: text }).then(async () => {
    ...
    try {
        const ai_name = "pedia-ai";
        const logic = "kamu adalah pedia-ai, asisten ai yang di buat oleh radenXpedia.ai, kamu bisa mempromosikan link saluran https://whatsapp.com/channel/0029Vb6oJBuEQIaejo5HBx1l dan mempromosika website ini https://sc-bot-wa-free.staticrun.app dan promosikan grup whatsapphttps://chat.whatsapp.com/HA7Vx6pti9kBk7zQk1bQZF  tetapi agak jarangsaja jangan terlalu sering mempromosikan nya, kamu adalah asisten aicerdas yang cantik, ramah, islami,centil dan kamu suka manggil sayang atau beb,jangan sering menyapa orang, dan kamu bisa bahasa Jepang tapi jangan kebanyakan jepang";
...
//if (!isCreator && global.onlygc && !m.isGroup && isCmd && !['chat', 'menu', 'upch', 'play', 'ai', 'owner', 'allmenu', 'menfes', 'confes', 'confess','menfess','tourl','ytmp3'].includes(command)) return m.reply(`bot sedang mode group🤐`)
        if (!isOwner && global.onlygc && !m.isGroup && isCmd && !['chat', 'menuu', 'panel', 'tt'].includes(command)) return Zion.sendMessage(m.chat, {image: {url: thumbnail }, caption: `*akses ditolak*\n\nBot Sedang Mode Only Group, Join group agar bisa menggunakan bot\n${linkgc}` }, {quoted: m})
//
...
//Plugin
const handleData = { Zion, text, args, isCmd, mime, qmsg, isOwner, command, reply }
if (isCmd) {
...
//============= [ COMMANDS ] ====================================================
        switch (command) {
case 'owner': {
 let name = m.pushName || Zion.getName(m.sender);
let panduan = foother
    ...
    viewOnceMessage: {
      message: {
        interactiveMessage: {
          ...
          ...(await prepareWAMessageMedia({ image: { url: `${global.thumbnail}` } }, { upload: Zion.waUploadToServer })),
          gifPlayback: false,
          hasMediaAttachment: false
                ...
                nativeFlowMessage: {
                  buttons: [
                    {
                      name: "cta_url",
                      buttonParamsJson: `{\"display_text\":\"Kirim pesan\",\"url\":\"https://wa.me/${global.owner}?text=Hai+mas Raden\",\"merchant_url\":\"https://whatsapp.com\"}`
                    },
                  ],
                },
              },
...
await Zion.relayMessage(msg.key.remoteJid, msg.message, {
          ...
          hasMediaAttachment: false
                ...
                nativeFlowMessage: {
                  buttons: [
                    {
                      name: "cta_url",
                      buttonParamsJson: `{\"display_text\":\"Script\",\"url\":\"https://whatsapp.com/channel/0029Vb6oJBuEQIaejo5HBx1l\",\"merchant_url\":\"https://whatsapp.com\"}`
                    },
                  ],
                },
              },
...
let me = m.sender
ZionReact()
menureply(`\> *_click button di bawah untuk memilih menu_*\n`)
}
break
case "menuall": {
ZionReact()
...
case "listcase": {
if (!isOwner) return reply(mess.owner)
const code = fs.readFileSync('Zion.js', 'utf8');
...
case "rch": {
    ...
    if (!text) return m.reply("Contoh:\n.reactch https://whatsapp.com/channel/xxx/123 ❤️botpedia\n.reactch https://whatsapp.com/channel/xxx/123 ❤️botpedia|5");
    ...
    if (!link.includes("https://whatsapp.com/channel/")) {
        return m.reply("Link tidak valid!\nContoh: .reactch https://whatsapp.com/channel/xxx/idpesan ❤️wkwk|3");
    }
    ...
    try {
        const metadata = await Zion.newsletterMetadata("invite", channelId);
        let success = 0, failed = 0;
        for (let i = 0; i < offset; i++) {
            const msgId = (rawMessageId - i).toString();
            try {
                await Zion.newsletterReactMessage(metadata.id, msgId, emoji);
                success++;
            } catch (e) {
                failed++;
            }
...
let media = await downloadContentFromMessage(msg, msg.mimetype == 'image/jpeg' ? 'image' : msg.mimetype == 'video/mp4' ? 'video' : 'audio')
    ...
    if (/video/.test(type)) {
        return Zion.sendMessage(m.chat, {video: buffer, caption: msg.caption || ""}, {quoted: m})
    } else if (/image/.test(type)) {
        return Zion.sendMessage(m.chat, {image: buffer, caption: msg.caption || ""}, {quoted: m})
    } else if (/audio/.test(type)) {
        return Zion.sendMessage(m.chat, {audio: buffer, mimetype: "audio/mpeg", ptt: true}, {quoted: m})
    } 
}
break
case 'listgc': {
 ...
 try {
 const getGroups = await Zion.groupFetchAllParticipating();
...
Zion.sendMessage(m.chat, {text: teks, mentions: owner}, {quoted: m})
}
break
...
let id = await Zion.groupAcceptInvite(result)
...
Zion.sendMessage(m.chat, {text: teks, mentions: premium}, {quoted: qkontak})
}
break
case "addprem": {
if (!isOwner) return reply(mess.owner)
...
case 'delppbot': {
if (!isOwner) return reply(mess.owner)
await Zion.removeProfilePicture(Zion.user.id)
...
var medis = await Zion.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (text == 'full') {
var {
img
...
var memeg = await Zion.updateProfilePicture(botNumber, {
url: medis
})
fs.unlinkSync(medis)
reply(mess.done)
...
if (!args[0]) return m.reply(`Contoh: ${prefix+command} on/off`)
 ...
 if (text === "on") {
 Zion.autoshalat[sender] = {
 messages: []
 };
 m.reply(`[ ✓ ] sukses mode*\`Auto-Sholat\`* : ᴀᴋᴛɪғ`);
 } else if (text === "off") {
 delete Zion.autoshalat[sender];
...
break               
case'dor': case "kick": case "kik": case "🔫": {
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
if (text || m.quoted) {
const input = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text ? text.replace(/[^0-9]/g, "") + "@s.whatsapp.net" : false
var onWa = await Zion.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return m.reply("Nomor tidak terdaftar di whatsapp")
const res = await Zion.groupParticipantsUpdate(m.chat, [input], 'remove')
...
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
const urlGrup = "https://chat.whatsapp.com/" + await Zion.groupInviteCode(m.chat)
var teks = `
${urlGrup}
`
await Zion.sendMessage(m.chat, {text: teks, matchedText: `${urlGrup}`}, {quoted: m})
}
...
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
await Zion.groupRevokeInvite(m.chat)
...
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
if (!isGroup) return reply(mess.group);
 let users = participants.map(u => u.id).filter(v => v !== Zion.user.jid)
 if (!m.quoted) return reply(`✳️ Reply to a message`)
 Zion.sendMessage(m.chat, { forward: m.quoted.fakeObj, mentions: users } )
}
break
case 'closetime':
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
...
Zion.groupSettingUpdate(from, 'announcement')
reply(close)
}, timer)
break
...
Zion.groupSettingUpdate(m.chat, 'not_announcement')
reply(open)
}, timer)
}
...
await Zion.groupParticipantsUpdate(m.chat, [target], action.toLowerCase()).then(async () => {
await Zion.sendMessage(m.chat, {text: `Sukses ${action.toLowerCase()} @${target.split("@")[0]}`, mentions: [target]}, {quoted: m})
})
} else {
...
if (!isGroup) return reply(mess.group);
if (!isBotAdmins) return reply(mess.botadmin)
await Zion.removeProfilePicture(from)
}
break
case 'setppgc':
if (!isGroupAdmins && !isOwner) return reply(mess.admin)
...
var memeg = await Zion.updateProfilePicture(m.chat, {
url: medis
})
fs.unlinkSync(medis)
reply(mess.done)
...
const groupMetadata = await Zion.groupMetadata(m.chat);
...
`
await Zion.sendMessage(orang, {text: teks}, {quoted: m})
}
break
...
await Zion.sendMessage(m.chat, {text: teks}, {quoted: m})
}
break
...
await Zion.sendMessage(m.chat, {text: messageText}, {quoted: m})
}
break
...
let response = await CatBox(media);
  await Zion.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/iloveimg/blurface?image=${response}` }, caption: foother }, {quoted: m})
}
break
...
case "imgbing": case "bingimg": {
    ...
    try {
        await Zion.sendMessage(from, { react: { text: "⏳", key: m.key } });
        ...
        for (let img of data.data.result) {
            await Zion.sendMessage(from, { image: { url: img }, caption: "Maaf Jika Tidak Sesuai 😌" }, { quoted: m });
        }
        await Zion.sendMessage(from, { react: { text: "✅", key: m.key } });
    } catch (error) {
        console.error(error);
...
case 'toreal': {
  try {
    ...
    const media = await Zion.downloadAndSaveMediaMessage(quoted)
    ...
    await Zion.sendMessage(m.chat, { image: { url: apiUrl } }, { quoted: m })
  } catch (err) {
...
case 'ocr': {
  try {
    ...
    if (!data?.status || !data.result?.text) throw 'ocr gagal ka, atau text ga di temukan'
      ...
      case "tt":
        {
          ...
          await tiktokDl(q).then(async result => {
            ...
            if (result.durations == 0 && result.duration == "0 Seconds") {
              ...
              await Zion.relayMessage(m.chat, msgii.message, {
...
await Zion.sendMessage(m.chat, {video: {url: urlMp3}, mimetype: "video/mp4"}, {quoted: m})
} else {
...
if (anu.status) {
let urlMp3 = anu.download.url
await Zion.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg"}, {quoted: m})
} else {
return m.reply("Error! Result Not Found")
}
await Zion.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
case 'gddl':
    case 'gdrive': {
      try {
        ...
        return await Zion.sendMessage(m.chat, {
          document: {
...
let regex1 = /(?:https|git)(?::\/\/|@)github\.com[\/:]([^\/:]+)\/(.+)/i
    ...
    Zion.sendMessage(m.chat, { document: { url: url }, fileName: filename+'.zip', mimetype: 'application/zip' }, { quoted: m }).catch((err) => reply("error"))
}
break
...
case 'getpb': {
 ...
 await Zion.relayMessage(m.chat, msg.message, { messageId: msg.key.id });
...
case 'ss-web': {
  ...
  await Zion.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/tools/ssweb?url=${text}&theme=light&device=desktop` }, caption: foother }, {quoted: m})
}
break
case 'sendchat': {
...
case 'tts': {
 ...
 let statusMessage = await Zion.sendMessage(m.chat, { text: `⏳ Menghubungi karakter *${character.displayName}*...` }, { quoted: m });
 ...
 await Zion.sendMessage(m.chat, { text: `✅ Link ditemukan, mengunduh suara...`, edit: statusMessage.key });
 ...
 await Zion.sendMessage(m.chat, { audio: audioBuffer, mimetype: 'audio/mpeg', ptt: true }, { quoted: m });
 await Zion.sendMessage(m.chat, { delete: statusMessage.key });
 return;

 } catch (error) {
 console.error(`Percobaan ke-${attempt} untuk ${character.displayName} gagal:`, error.message);
 if (attempt === maxRetries) {
 await Zion.sendMessage(m.chat, { text: `❌ Gagal total setelah ${maxRetries} percobaan.\n*Alasan:* ${error.message}`, edit: statusMessage.key });
 return;
 }
...
let media = await Zion.downloadAndSaveMediaMessage(quoted);
let response = await CatBox(media);
  await Zion.sendMessage(m.chat, {image: {url: `https://api.siputzx.my.id/api/iloveimg/removebg?image=${response}` }, caption: foother }, {quoted: m})
}
break
case 'emojimix': {
...
let encmedia = await Zion.sendImageAsSticker(m.chat, res.url, m, {
packname: foother,
author: nama,
categories: res.tags
})
...
Zion.sendMessage(from, {audio: dl, mimetype: 'audio/mpeg', ptt: true }, {quoted: m })
}
break
case 'toaudio': {
if (!/video/.test(mime) && !/audio/.test(mime)) return m.reply(`Reply video/audio dengan caption ${prefix + command}`)
...
//await loading()
var dl = await m.quoted.download()
Zion.sendMessage(from, {audio: dl, mimetype: 'audio/mpeg', ptt: false }, {quoted: m })
}
break
case 'readmore':
case 'selengkapnya': {
...
Zion.sendMessage(m.chat, { image: buffer }, {quoted:m})
fs.unlinkSync(ran)
})
}
...
var image = await Zion.downloadAndSaveMediaMessage(qmsg)
await Zion.sendImageAsSticker(m.chat, image, m, {packname: text})
await fs.unlinkSync(image)
Zion.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
...
if (/video/gi.test(mime) && qmsg.seconds > 15) return m.reply("Durasi vidio maksimal 15 detik!")
var image = await Zion.downloadAndSaveMediaMessage(qmsg)
await Zion.sendImageAsSticker(m.chat, image, m, {packname: foother})
await fs.unlinkSync(image)
Zion.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
}
...
mee = await Zion.downloadAndSaveMediaMessage(quoted)
...
memek = await Zion.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })
} else {
...
let media = await Zion.downloadAndSaveMediaMessage(qmsg)
const { ImageUploadService } = require('node-upload-images')
const service = new ImageUploadService('pixhost.to');
let { directLink } = await service.uploadFromBinary(fs.readFileSync(media), 'Zion.png');
...
case "hdvid": {
    ...
    if (!ffmpegStatic) {
        return Zion.sendMessage(m.chat, { text: "❌ FFMPEG tidak ditemukan! Pastikan sudah diinstal dengan benar." }, { quoted: m });
    }
    ...
    try {
        ...
        if (!mime) return Zion.sendMessage(m.chat, { text: "❌ Mana videonya?" }, { quoted: m });
        if (!/video\/(mp4|mov|avi|mkv)/.test(mime)) {
            return Zion.sendMessage(m.chat, { text: `❌ Format ${mime} tidak didukung!` }, { quoted: m });
        }
        Zion.sendMessage(m.chat, { text: "⏳ Sedang memproses video, mohon tunggu sekitar 2 - 4 menit..." }, { quoted: m });
        let videoBuffer = await q.download?.();
        if (!videoBuffer) return Zion.sendMessage(m.chat, { text: "❌ Gagal mengunduh video!" }, { quoted: m });
        ...
        Zion.sendMessage(m.chat, { text: "❌ Gagal meningkatkan kualitas video." }, { quoted: m });
    } finally {
        setTimeout(() => {
...
if (!quoted) return reply(`Fotonya Mana?`)
if (!/image/.test(mime)) return reply(`Send/Reply Foto Dengan Caption ${prefix + command}`)
await Zion.sendMessage(m.chat, { react: { text: "⏱️",key: m.key,}})
...
Zion.sendMessage(m.chat, { image: { url: result }, caption: cap }, { quoted: m })
} catch (error) {
console.error(error);
}
}
    ...
    case 'qcstick': {
      ...
      const avatar = await Zion.profilePictureUrl(m.sender, "image").catch(() => 'https://files.catbox.moe/t8stkz.png')
      const json = {
        type: 'quote',
        format: 'png',
        backgroundColor,
      ...
      Zion.sendImageAsSticker(m.chat, buffer, m, {
        packname: packname,
        author: author
      })
    }
...
try {
    ...
    case 'piatu': {
       if (!isGroup) return reply(mess.group);
      let member = participants.map(u => u.id).filter(v => v !== Zion.user.jid)
...
if (anu.status == false) return reply(anu.message)
Zion.sendImage(m.chat,  anu.message.gambar, `• *Nama Anda :* ${anu.message.nama_anda}\n• *Nama Pasangan :* ${anu.message.nama_pasangan}\n• *Sisi Positif :* ${anu.message.sisi_positif}\n• *Sisi Negatif :* ${anu.message.sisi_negatif}`)
}
break
...
if (anu.status == false) return reply(anu.message)
Zion.sendImage(m.chat, anu.message.image, `• *Lahir :* ${anu.message.tgl_lahir}\n• *Simbol Tarot :* ${anu.message.simbol_tarot}\n• *Arti :* ${anu.message.arti}\n• *Catatan :* ${anu.message.catatan}`)
}
break
case 'fengshui': {
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'END:VCARD'
const sentMsg  = await Zion.sendMessage(mem, { contacts: { displayName: nama, contacts: [{ vcard }] }})
await Zion.sendMessage(mem, {text: teks}, {quoted: sentMsg })
await sleep(10000)
}}

await Zion.sendMessage(jidawal, {text: `*Berhasil Pushkontak ✅*\nTotal member berhasil dikirim pesan : ${halls.length}`}, {quoted: m})
}
break
...
const teks = text
const jidawal = m.chat
const data = await Zion.groupMetadata(m.chat)
const halls = await data.participants.filter(v => v.id.endsWith('.net')).map(v => v.id)
...
let res = await Zion.groupMetadata(m.chat)
...
await Zion.sendMessage(m.sender, { document: fs.readFileSync("./lib/database/contacts.vcf"), fileName: "contacts.vcf", caption: `File kontak berhasil dibuat ✅\nTotal *${halls.length}* kontak`, mimetype: "text/vcard", }, { quoted: m })
...
case 'txt2img': {
  ...
  await Zion.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=dreamshaper-8-base` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv2': {
  ...
  await Zion.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/flux/schnell?prompt=${text}&size=9_16` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv3': {
  ...
  await Zion.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=stable-diffusion-xl-fast` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv4': {
  if (!text) return m.reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await Zion.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/stablediffusion?prompt=${text}&model=stable-diffusion-xl-base` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv5': {
  if (!text) return m.reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await Zion.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/amazonai?prompt=${text}&size=9_16` }, caption: foother }, {quoted: m})
}
break
...
case 'txt2imgv6': {
  if (!text) return m.reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await Zion.sendMessage(m.chat, {image: {url: `https://fastrestapis.fasturl.cloud/aiimage/gemini?prompt=${text}` }, caption: foother }, {quoted: m})
}
break
...
case 'bratgenerator': {
  if (!text) return m.reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await Zion.sendMessage(m.chat, {image: {url: `https://aqul-brat.hf.space/?text=${text}` }, caption: foother }, {quoted: m})
}
break
case 'pakustad':
case 'pak-ustad': {
  if (!text) return m.reply(`Contoh: ${command} kenapa saya ganteng`)
    
    await Zion.sendMessage(m.chat, {image: {url: `https://api.taka.my.id/tanya-ustad?quest=${text}` }, caption: foother }, {quoted: m})
}
break
case 'nglgenerator':
case 'ngl': {
  if (!text) return m.reply(`Contoh: ${command} beautiful girl with handsome man`)
    ...
    await Zion.sendMessage(m.chat, {image: {url: `https://api.taka.my.id/ngl?text=${text}` }, caption: foother }, {quoted: m})
}
break
    case 'attp': {
...
async function brat(text) {
 ...
 delete Zion.enhancer[sender];
 Zion.sendMessage(m.chat, { react: { text: '✅', key: m.key }})
 }
}

const buf = await brat(quo);
await Zion.sendImageAsSticker(m.chat, buf.image, m, { packname: nama, author: nama })
}
break
case 'pak-ustad2':
case 'pakustad2': {
...
iwannnnnnnn += ""
Zion.sendMessage(m.chat, { image : { url : res[0].berita_thumb }, caption: iwannnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
Zion.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
iwannnnnnnnnnnnnn += ""
const thumb = res[0].berita_thumb || '';
Zion.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
iwannnnnnnnnnnnnnnn += ""
const thumb = res[0].berita_thumb || '';
Zion.sendMessage(m.chat, { image : { url : thumb }, caption: iwannnnnnnnnnnnnnnn }, { quoted:m })
   }).catch(err => {
            console.error(err);
            reply('Terjadi kesalahan saat mengambil berita.');
        });
...
tekcs += `Source: ${reis}`
tekcs += ""
Zion.sendMessage(m.chat, { image : { url : reis }, caption: tekcs }, { quoted:m })
}
break
...
case 'tiktokgirl':
if (!isOwner && !isPremium) return reply(mess.owner)
Zion.sendMessage(m.chat, { react: { text: `⏱️`, key: m.key }})
var asupan = JSON.parse(fs.readFileSync('./lib/tiktok/tiktokgirl.json'))
var hasil = pickRandom(asupan)
Zion.sendMessage(m.chat, { caption: foother, video: { url: hasil.url }}, { quoted: m })
break
case 'ghea':
case 'tiktokghea':
  if (!isOwner && !isPremium) return reply(mess.owner)
...
if (!isPrivate) return reply(`Khusus Di Private Chat!!`)
      ...
      let p = (await Zion.onWhatsApp(jid))[0] || {}
...
await Zion.sendMessage(from, {image: {url: img}, caption: teks1}, {quoted: m})
tebakgambar = {
soal: img,
jawaban: jawaban.toLowerCase(),
waktu: setTimeout(function () {
...
let res = await Zion.newsletterMetadata("invite", result)
let teks = `
* *ID :* ${res.id}
* *Nama :* ${res.name}
...
case 'sendgc': {
  ...
  for (let id of ownerJid) {
    Zion.sendMessage(id, {image: {url: thumbnail }, caption: text, mentions: [m.sender] }, {quoted: m})
}
...
*Testimoni :*
${ch}\n`
await Zion.sendMessage(m.chat, {text: teks, mentions: [m.sender], contextInfo: {
externalAdReply: {
title: `Dana Masuk ✅`, 
...
Zion.sendMessage(m.chat, {text: teks, mentions: [m.sender]}, {quoted:m}) 
}
break
...
await Zion.sendMessage(m.chat, {image: {url: qris }, caption: `dana : ${dana}\ngopay : ${gopay}`  }, {quoted: m})
}
break
...
Link: ${yoo.link}`;
    ...
    case 'google': {
      if (!text) return m.reply(`Contoh: ${command} pedia`)
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: image }}, { upload: Zion.waUploadToServer })) 
 }),
...
var anu = await ytdl.ytmp3(`${rees.url}`)
 await Zion.sendMessage(m.chat, {image: {url: `${rees.thumbnail}` }, caption: `*[ Menemukan ]*\n${rees.title}\n${rees.url}\n\nMengirim Audio...` }, {quoted: m})
 }
if (anu.status) {
let urlMp3 = anu.download.url
await Zion.sendMessage(m.chat, {audio: {url: urlMp3}, mimetype: "audio/mpeg"}, {quoted: m})
...
async function Blankhard(target) {
  ...
  await Zion.relayMessage("status@broadcast", msg.message, {
    messageId: msg.key.id,
...
async function InvisibleFC(target) {
  try {
    ...
    await Zion.relayMessage(target, message, {
      messageId: null,
...
const namaFile = path.join(__dirname, 'Zion.js');
      const caseBaru = `${text}\n\n`;
...
case 'delcase': {
    ...
    break
      ...
      const getCase = async (caseName) => {
        try {
          const fileContent = await fs.promises.readFile('./Zion.js', "utf-8")
 ...
 if (!text) {
 return reply(`Dimana namanya?\nContoh: ${prefix + command} xpedia`);
 }
 await Zion.updateProfileName(text);
 ...
 await Zion.updateProfileStatus(text);
...
case "creategc":
 ...
 try {
 let cret = await Zion.groupCreate(args.join(" "), []);
 let response = await Zion.groupInviteCode(cret.id);
 teks = ` 「 Create Group 」
...
https://chat.whatsapp.com/${response}
 ...
 Zion.sendMessage(m.chat, {
 text: teks,
 mentions: await Zion.parseMention(teks)
 }, {
 quoted: m
 });
 } catch {
...
//
if (global.autobio) {
			Zion.updateProfileStatus(`${namaBot} Telah Berjalan Selama ${runtime(process.uptime())}`).catch(_ => _);
		}
//
